...

{
	...
	while(1) {
		message++;
		sprintf(strings, "%s-%d ", argv[2], message);
		if((numbytes = sendto(sockfd, strings, strlen(strings), 0, (struct sockaddr *)&their_addr, sizeof(struct sockaddr))) == -1)
		{
